...
must_haves:
  truths:
    - "BugService calculates all 9 KPIs (KPI-01 through KPI-09) matching specification formulas"
...
Purpose: Complete the backend foundation by implementing all 9 KPI calculations (KPI-01 through KPI-09) and exposing them via REST API. This enables the frontend dashboard to display bug metrics.
...
<task type="auto">
  <name>Task 1: Add KPI calculations, upload workflow, and query methods to BugService</name>
  <files>server/services/BugService.js</files>
  <action>
Add the following methods to BugService.js (extends foundation from 10-01):
- calculateKPIs, calculateMedian, calculateStdDev, getWeekKey (KPI math)
- uploadCSV (full upload workflow with transaction)
- listBugs, getKPIs (query methods for API routes)

**1. calculateKPIs method - calculates all 9 KPIs (KPI-01 through KPI-09) from bug array:**
...
# Verify all 9 KPIs (KPI-01 through KPI-09) are calculated
grep -E "bug_inflow_rate|mttr_by_priority|sla_vh_percent|backlog_health_score" server/services/BugService.js | wc -l
# Should be >= 4
```
  </verify>
  <done>BugService has calculateKPIs with all 9 KPIs (KPI-01 through KPI-09), uploadCSV with transaction, listBugs with filtering, getKPIs methods</done>
</task>
...
1. **KPI Calculations (KPI-01 through KPI-09):**
   - BugService.calculateKPIs returns all 9 KPIs (KPI-01 through KPI-09)
...
<success_criteria>
- [ ] BugService.calculateKPIs calculates all 9 KPIs (KPI-01 through KPI-09)
